...
export default function Game() {
    const { cols, gridSize, maxMines, rows } = useGame();
        ...
        while (minesCount < maxMines) {
            ...
            if (!map[i].mine) {
                ...
                neighbors.forEach((neighbor) => {
                    if (isValid(neighbor, i, cols, gridSize)) {
    ...
    const openBlock = (block: Block, cont: Boolean = true) => {
        ...
        const getNeighbors = (i: number) => {
            const neighbors = [i + 1, i - 1, i + cols, i - cols, i + cols + 1, i - cols - 1, i + cols - 1, i - cols + 1];
            return neighbors.filter((_) => isValid(_, i, cols, gridSize));
        };
    ...
    return (
        <div onContextMenu={(event) => event.preventDefault()}>
            <GameStats gameState={gameState} restart={restart} flags={flags} opened={opened} startTime={startTime} />
            <div className={`flex flex-col items-center justify-center relative pt-24`}>
                <div
                    className="gap-0 shadow-2xl rounded-2xl"
                    style={{
                        display: "grid",
